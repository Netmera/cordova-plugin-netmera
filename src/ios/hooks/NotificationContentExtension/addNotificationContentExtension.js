...
const addExtensionToProject = (contextHelper, project) => {
  ...
  logHelper.warn("enis1")
  logHelper.warn(JSON.stringify(existingServiceExtensions));

  // Message user if another extension that is not ours is found
  if (existingServiceExtensions.find(x => projectHelper.unquote(x.name) === ourServiceExtensionName)) {
    logHelper.warn('You already have a notification service extension. Please follow our guide to support rich push notifications');
  ...
  return fs.copy(source, destination)
    ...
    .then(() => {
      ...
      const filePaths = [
        ...
        'netmerapushnotificationcontentextension-debug.xcconfig',
        'netmerapushnotificationcontentextension-release.xcconfig',
      ...
      for (const buildConfiguration of buildConfigurations) {
        ...
        const baseConfigurationFileReference = projectHelper.findFileByName(`netmerapushnotificationcontentextension-${environment.toLowerCase()}.xcconfig`);
